...
class FlowLanguageClient extends AutoLanguageClient {
  ...
  startServerProcess() {
    ...
    const command = [path.join(SERVER_HOME, 'cli.js'), '--node-ipc'];
    if (config.tryFlowBin) {
      command.push('--try-flow-bin');
    }
    if (config.customFlowPath.enabled && config.customFlowPath.flowPath) {
      command.push('--flow-path', config.customFlowPath.flowPath);
    }
    if (!config.autoDownloadFlowBinaries) {
      command.push('--no-auto-download');
    }
